...
import css from './TaPresentationCard.module.css'

const TA_PRESENTATION_MARKER = '[DSH_TA_PRESENTATION_V1]'
...
export type TaPresentationCardProps = ToolCallViewProps & PropsLocale<'desktop.taPresentation'>

export const TA_DIRECT_PRESENTATION_TOOLS = [
  'ta_retention_analyze',
] as const
...
function isPresentation(value: unknown): value is TaPresentationModel {
  if (!isRecord(value)
    ...
    || typeof value.sourceTool !== 'string' || value.sourceTool.length > MAX_TEXT_LENGTH
...
function directRetentionPresentation(result: unknown): TaPresentationModel | undefined {
  if (!isRecord(result)) return undefined
  const data = isRecord(result.data) ? result.data : result
  if (!Array.isArray(data.x) || !data.x.every(value => typeof value === 'string') || !isRecord(data.y)) return undefined
  const dates = data.x as string[]
  ...
  const byDate = [data.y, ...Object.values(data.y).filter(isRecord)]
  ...
  for (const date of dates.slice(0, MAX_HEATMAP_ROWS)) {
    ...
    const initial = numericValue(entity.initNum)
    const first = numericValue(entity.values[0])
    const rawValues = initial !== null && first === initial && entity.values.length > 1
      ? entity.values.slice(1)
      : entity.values
    const countValues = initial !== null && initial > 0
      && rawValues.some(value => (numericValue(value) ?? 0) > 1)
    truncated ||= rawValues.length > MAX_HEATMAP_COLUMNS
    const values = rawValues.slice(0, MAX_HEATMAP_COLUMNS).map((value) => {
      const numeric = numericValue(value)
      if (numeric === null) return null
      if (countValues && initial !== null && initial > 0) return numeric / initial * 100
      return numeric > 1 ? numeric : numeric * 100
    })
    rows.push({ label: date, ...(initial === null ? {} : { initial }), values })
  }
  if (rows.length === 0) return undefined
  const width = Math.min(Math.max(...rows.map(row => row.values.length)), MAX_HEATMAP_COLUMNS)
  const generatedAt = typeof data.result_generate_time === 'string' ? data.result_generate_time : undefined
  return {
    schemaVersion: 1,
    view: 'heatmap',
    title: '留存分析',
    ...
    truncated,
    payload: {
      columns: Array.from({ length: width }, (_, index) => `D${String(index)}`),
      rows,
      format: 'percent',
    },
  }
...
export function presentationFromBlock(
  block: TaPresentationCardProps['block'],
  toolName = 'ta_present',
): TaPresentationModel | undefined {
  if (!('kind' in block)) return undefined
  if (isPresentation(block.meta)) return block.meta
  const markerText = textContent(block.content)
  const index = markerText.indexOf(TA_PRESENTATION_MARKER)
  if (index >= 0) {
    try {
      const parsed = JSON.parse(markerText.slice(index + TA_PRESENTATION_MARKER.length)) as unknown
      return isPresentation(parsed) ? parsed : undefined
    } catch {
      return undefined
    }
  }
  const rawToolName = toolName.slice(toolName.lastIndexOf('__') + 2)
  if (!TA_DIRECT_PRESENTATION_TOOLS.includes(rawToolName as typeof TA_DIRECT_PRESENTATION_TOOLS[number])) {
    return undefined
  }
  try {
    const result = JSON.parse(markerText) as unknown
    const model = rawToolName === 'ta_retention_analyze' ? directRetentionPresentation(result) : undefined
    return isPresentation(model) ? model : undefined
  } catch {
    return undefined
  }
}
...
function cartesianData(payload: TaCartesianPayload) {
...
function heatmapData(payload: TaHeatmapPayload) {
...
function LineChart({ payload, label }: { readonly payload: TaCartesianPayload; readonly label: string }): ReactNode {
...
function BarChart({ payload, label }: { readonly payload: TaCartesianPayload; readonly label: string }): ReactNode {
  const data = cartesianData(payload)
  if (data.length === 0) return <EmptyChart label={label} />
  return (
    <AntvFrame label={label}>
...
function FunnelChart({ payload, label }: { readonly payload: TaFunnelPayload; readonly label: string }): ReactNode {
  ...
  return (
    <AntvFrame label={label}>
      <Funnel
        ...
        data={payload.steps}
        label={{
          text: (datum: TaFunnelPayload['steps'][number]) => `${datum.label}\n${formatValue(datum.value)}`,
...
function HeatmapChart({ payload, label }: { readonly payload: TaHeatmapPayload; readonly label: string }): ReactNode {
...
function SankeyChart({ payload, label }: { readonly payload: TaSankeyPayload; readonly label: string }): ReactNode {
...
function tableOf(model: TaPresentationModel): TaTablePayload {
  switch (model.view) {
    case 'line':
    case 'bar': {
    ...
    case 'funnel':
      return {
        columns: ['Step', 'Value'],
        rows: (model.payload as TaFunnelPayload).steps.map(step => [step.label, step.value]),
      }
    case 'heatmap': {
      const payload = model.payload as TaHeatmapPayload
      return {
    ...
    case 'sankey':
      return {
        ...
        rows: (model.payload as TaSankeyPayload).links.map(link => [link.source, link.target, link.value]),
      }
    case 'table':
      return model.payload as TaTablePayload
  }
}

function DataTable({ payload, label }: { readonly payload: TaTablePayload; readonly label: string }): ReactNode {
  return (
...
function PresentationChart({ model, label }: {
  readonly model: TaPresentationModel
  readonly label: string
}): ReactNode {
  switch (model.view) {
    case 'line': return <LineChart payload={model.payload as TaCartesianPayload} label={label} />
    case 'bar': return <BarChart payload={model.payload as TaCartesianPayload} label={label} />
    case 'funnel': return <FunnelChart payload={model.payload as TaFunnelPayload} label={label} />
    case 'heatmap': return <HeatmapChart payload={model.payload as TaHeatmapPayload} label={label} />
    case 'sankey': return <SankeyChart payload={model.payload as TaSankeyPayload} label={label} />
    case 'table': return <DataTable payload={model.payload as TaTablePayload} label={label} />
  }
}

function rawError(block: Extract<TaPresentationCardProps['block'], { kind: 'tool-result' }>): string {
  const text = textContent(block.content).trim()
  return text.length > 0 ? text : block.error?.code ?? 'Unknown presentation error'
}

export function TaPresentationCard({ block, inspect, toolName, t }: TaPresentationCardProps): ReactNode {
  const [expanded, setExpanded] = useState(true)
  const [tab, setTab] = useState<'chart' | 'data'>('chart')
  const model = useMemo(() => presentationFromBlock(block, toolName), [block, toolName])
      ...
      <header className={css.header}>
        <button className={css.headingButton} type="button" aria-expanded={expanded} onClick={() => { setExpanded(value => !value) }}>
          ...
          <span className={css.headingCopy}>
            <strong>{label}</strong>
            <span>{!settled ? t('preparing') : failed ? t('failed') : model === undefined ? t('invalid') : model.sourceTool}</span>
          </span>
