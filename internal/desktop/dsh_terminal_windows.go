...
func terminalBootstrap(config dshTerminalConfig, binDir string) (string, error) {
	values := []string{config.StateDirectory, config.HarnessHome, config.Node, config.PNPM, config.UV, config.PNPMStore, config.CLI, binDir, filepath.Dir(config.Node), filepath.Dir(config.PNPM), config.WorkingDirectory}
	...
	toolPath := escape(binDir) + `;` + escape(filepath.Dir(config.Node)) + `;` + escape(filepath.Dir(config.PNPM))
	if config.UV != "" {
		toolPath += `;` + escape(filepath.Dir(config.UV))
	}
