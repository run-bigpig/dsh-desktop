// Adapted from open-pencil/open-pencil v0.14.0, src/app/editor/vector-edit/network.ts
...
import type { NodeEditState, VectorEditState } from './types.ts'
...
export function createVectorEditNetworkActions(
  ...
  function nodeEditConnectEndpoints(a: number, b: number) {
    ...
    const remap = (idx: number): number => {
      if (idx === removeIndex) return keepIndex > removeIndex ? keepIndex - 1 : keepIndex
