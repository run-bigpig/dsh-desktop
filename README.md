...
Windows 发布相关改动还应通过 Windows PowerShell 覆盖相应包测试，并构建 host、client、bundle 和 design-ui 四个内置插件包。插件源代码位于 [plugin/packages](plugin/packages)，统一构建入口为 [plugin/scripts/build-against-harness.mjs](plugin/scripts/build-against-harness.mjs)。设计画布使用 OpenPencil 官方嵌入 SDK，在构建时编译进 plugin-host，不依赖独立 UI Release。
