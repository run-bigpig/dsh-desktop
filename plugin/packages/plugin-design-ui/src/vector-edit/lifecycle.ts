// Adapted from open-pencil/open-pencil v0.14.0, src/app/editor/vector-edit/lifecycle.ts
...
import { getLiveNetwork } from './network.ts'
...
export function createVectorEditLifecycle(editor: Editor, state: VectorEditState) {
  ...
  function applyNodeEditToNode(es: NonNullable<typeof state.nodeEditState>) {
    ...
    // StarWeave commits completed gestures while edit mode stays open, so
    // compare against current geometry rather than the entry snapshot.
    if (vectorNetworksEqual(getLiveNetwork(es), es.committedAbsNetwork ?? es.origAbsNetwork)) return
    ...
    es.committedAbsNetwork = cloneVectorNetwork(getLiveNetwork(es))
