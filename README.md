...
- 关闭窗口隐藏到托盘；支持单实例恢复、打开内置终端、重启、桌面版本更新检查、日志和退出。
...
桌面更新源固定为 `https://api.github.com/repos/run-bigpig/dsh-desktop/releases/latest`。Windows x64 Release 必须同时发布 `StarWeaveInstaller.exe` 和对应的 `.sha256` 文件；如果 GitHub API 已提供资产 digest，也会直接使用该 SHA-256。
...
GitHub Actions 的 `Windows Package` 工作流支持手动打包；推送 `v*` 标签时还会把安装器和校验文件发布到对应 GitHub Release，供应用内“检查桌面更新”使用。
