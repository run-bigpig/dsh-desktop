...
import { proxyFromEnv, shouldBypassProxy, fetchWithProxy } from "../../packages/plugin-host/src/web-tools/host/fetch-proxy.ts";
...
test("module loads and degrades to plain fetch when undici is unavailable", async () => {
  ...
  try {
    await fs.cp(
      new URL("../../packages/plugin-host/src/web-tools/host/fetch-proxy.ts", import.meta.url),
      tmp + "/fetch-proxy.ts",
    );
    const url = new URL("file:///" + (tmp + "/fetch-proxy.ts").replace(/\\/g, "/")).href;
    const script = `
      (async () => {
    ...
    `;
    const out = execFileSync(process.execPath, ["--experimental-strip-types", "-e", script], { encoding: "utf8", timeout: 15000 });
