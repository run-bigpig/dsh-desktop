...
- **内置桌面集成**：通过 Harness 正式插件扩展点提供插件市场、会话工作台、文件与 Git 操作、MCP、文档与视觉能力、图像工作台、图表展示、ThinkingData 配置、界面外观和 OpenPencil Companion 集成入口。
...
| pnpm | `11.7.0` |
| 内置桌面插件 | `0.1.75` |
| OpenPencil Companion | `0.14.0-starweave.2` |

权威锁定信息位于 [release/seed.lock.json](release/seed.lock.json)、[release/toolchain.lock.json](release/toolchain.lock.json) 和 [release/openpencil.lock.json](release/openpencil.lock.json)。Harness 或工具链升级必须同步相关 seed 锁文件，并重新完成官方构建、插件构建和 Windows 运行验证。
...
```bash
go test ./internal/appconfig ./internal/state ./internal/runtime ./internal/openpencil ./internal/update ./internal/selfupdate ./internal/logging ./internal/backup ./internal/seed
```
...
- [StarWeave OpenPencil](https://github.com/run-bigpig/starweave-openpencil)：随桌面发行版锁定的 OpenPencil Companion。
